...
% 	\[
% 		\mathfrak A(G) = m(G)/m(G)m(\ker\phi)
% 	\]
% 	mit dem Alexander-Ideal
% 	\[
% 		\mathfrak I(G) = E_1 (\mathfrak A(G)) \subset \ZZ[F]
% 	\]
% 	und dem Alexander-Polynom, so dass $(\Delta_M ) \supset \mathfrak I(G)$ das kleinste Hauptideal ist.
...
% 			\end{xy}			
% 		\end{equation}
% 		Die zweite Reihe ist exakt nach Lemma~\ref{lem:augmker} und die Exaktheit der ersten Reihe geht aus der Anwendung des Schlangenlemmas auf folgendes kommutatives Diagramm mit exakten Reihen, deren Spalten die Augmentationsabbildungen sind, hervor:
% 		\[
